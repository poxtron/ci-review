...
function vipgoci_exit_status( $results ) {
	foreach (
		...
		if ( ! isset( $results['stats'][ $stats_type ] )
...
function vipgoci_run() {
   ...
   */
	...
	if ( ! isset( $options['repo-owner'] )
		...
		print 'Usage: ' . $argv[0] . PHP_EOL . "\t"
		      ...
		      . 'mandatory, while others are optional.' . PHP_EOL . PHP_EOL . "\t"
		      . 'Note that if option --autoapprove is specified, --autoapprove-label needs to' . PHP_EOL . "\t" . 'be specified as well.'
		      . PHP_EOL . PHP_EOL . "\t" . '--repo-owner=STRING            Specify repository owner, can be an organization' . PHP_EOL
		      ...
		      . '                               start with https:// or https:// ' . PHP_EOL . PHP_EOL . "\t"
		      ...
		      . '                                                           See README.md for usage. ' . PHP_EOL . PHP_EOL . "\t"
		      ...
		      . '--set-support-level-label-prefix=STRING    Prefix to use for support level labels. Should be longer than five letters.'
		      . PHP_EOL . "\t" . '--repo-meta-api-base-url=STRING      Base URL to repo-meta API, containing support level and other '
		      . PHP_EOL . "\t" . '                                     information. ' . PHP_EOL . "\t"
		      . '--repo-meta-api-user-id=STRING       Authentication detail for the repo-meta API. ' . PHP_EOL . "\t"
		      . '--repo-meta-api-access-token=STRING  Access token for the repo-meta API. ' . PHP_EOL . PHP_EOL . "\t"
		      ...
		      . '                                                 Folders should be separated by newlines.' . PHP_EOL . PHP_EOL . "\t"
		      ...
		      . '                               paths to individual parts of the API.' . PHP_EOL . PHP_EOL . "\t"
		      ...
		      . '                               All required for hashes-to-hashes requests.' . PHP_EOL . PHP_EOL . "\t"
		      ...
		      . '--output=FILE                  Where to save output made from running PHPCS' . PHP_EOL . PHP_EOL . "\t"
		      ...
		      . '                                                separated by newlines.' . PHP_EOL . PHP_EOL . "\t"
		      ...
		      . '                               configuration file. Values are separated by commas. ' . PHP_EOL . PHP_EOL . "\t"
   ...
   */
	...
	if ( empty( $options['phpcs-runtime-set'] ) ) {
		...
		foreach (
			$options['phpcs-runtime-set'] as $tmp_runtime_key => $tmp_runtime_set
		) {
			$options
			['phpcs-runtime-set']
			[ $tmp_runtime_key ]
	   ...
	   */
			...
			if ( ( count(
				...
				vipgoci_sysexit(
					'--phpcs-runtime-set is incorrectly formed; it should ' . PHP_EOL . 'be a comma separated string of keys and values.'
					. PHP_EOL . 'For instance: --phpcs-runtime-set="foo1 bar1,foo2 bar2"',
					[
						$options['phpcs-runtime-set']
					],
					VIPGOCI_EXIT_USAGE_ERROR
  ...
  */
   ...
   */
	 ...
	 */
		...
		if ( ! isset(
			$tmp_repo_meta_match[ $options['post-generic-pr-support-comments-repo-meta-match'][ $i ][0] ]
		) ) {
			$tmp_repo_meta_match[ $options['post-generic-pr-support-comments-repo-meta-match'][ $i ][0] ] = [];
		}

		$tmp_repo_meta_match[ $options['post-generic-pr-support-comments-repo-meta-match'][ $i ][0] ][] = vipgoci_convert_string_to_type(
   ...
   */
	...
	if ( ( isset( $options['set-support-level-label-prefix'] ) )
   ...
   */
	if ( ! empty( $options['output'] ) ) {
		if ( ! is_string( $options['output'] ) ) {
			vipgoci_sysexit(
				'The --output argument should be a single string,' . 'but it looks like it is something else. Please check '
		...
		if ( is_dir( $options['output'] ) ) {
			vipgoci_sysexit(
				'The file specified in --output argument is invalid, ' . 'should not be a directory',
				[
					'output' => print_r( $options['output'], true ),
				],
				VIPGOCI_EXIT_USAGE_ERROR
	...
	if ( ( true === $options['autoapprove'] )
		...
		vipgoci_sysexit(
			'To be able to auto-approve, file-types to approve ' . 'must be specified, as well as a label; see --help ' . 'for information',
			[],
			VIPGOCI_EXIT_USAGE_ERROR
		);
	}
   ...
   */
	if ( ( true === $options['svg-checks'] )
		...
		vipgoci_sysexit(
			'--svg-checks is set to true, but no scanner is ' . 'configured. Please provide a valid path.',
			[],
			VIPGOCI_EXIT_USAGE_ERROR
		);
	}
   ...
   */
	...
	if ( ( true === $options['autoapprove'] )
		...
		vipgoci_sysexit(
			'PHP and JS files cannot be auto-approved on file-type basis, as they ' . 'can cause serious problems for execution',
			[],
			VIPGOCI_EXIT_USAGE_ERROR
		);
	}
   ...
   */
	...
	if ( ( true === $options['autoapprove'] )
		...
		vipgoci_sysexit(
			'SVG files cannot be auto-approved on file-type basis, as they ' . 'can contain problematic code. Use --svg-checks=true to '
   ...
   */
	 ...
	 */
		...
		if ( ( true === $options['lint'] )
		     && ( false === $options['phpcs'] ) ) {
			vipgoci_sysexit(
				'The current commit is not the latest one ' . 'to the Pull-Request, skipping ' . 'linting, and not doing PHPCS '
				. '-- nothing to do',
				[
		...
		} elseif ( ( true === $options['lint'] )
		           && ( true === $options['phpcs'] ) ) {
			// Skip linting, useless if not latest commit
			$options['lint'] = false;

			vipgoci_log(
				'The current commit is not the latest ' . 'one to the Pull-Request, ' . 'skipping linting',
				[
					'repo_owner' => $options['repo-owner'],
					'repo_name'  => $options['repo-name'],
					'pr_number'  => $pr_item->number,
   ...
   */
	...
	if ( ( ! empty( $options['pixel-api-url'] ) )
	 ...
	 * to all repositories.
	 */ $options['pixel-api-groupprefix'] . '-actions'                              => [
				'github_pr_approval',
	 ...
	 */ $options['pixel-api-groupprefix'] . '-' . $options['repo-name'] => [
				'github_pr_approval',
				'github_pr_non_approval',
   ...
   */
	vipgoci_log(
		...
		[
			...
			'run_time_measurements' => vipgoci_runtime_measure(
				VIPGOCI_RUNTIME_DUMP,
				null
			),
			'counters_report'       => $counter_report,

			'github_api_rate_limit' => $github_api_rate_limit_usage->resources->core,
