...
final class ApAutoApprovalTest extends TestCase {
	...
	private function pr_get_labels() {
		$github_url = VIPGOCI_GITHUB_BASE_URL . '/' . 'repos/' . rawurlencode( $this->options['repo-owner'] ) . '/' . rawurlencode(
				$this->options['repo-name']
			) . '/' . 'issues/' . rawurlencode( $this->options['pr-test-ap-auto-approval-1'] ) . '/' . 'labels';
		...
		foreach ( $data as $data_item ) {
			if ( $data_item->name === $this->options['autoapprove-label'] ) {
				return $data_item;
			}
		}
	...
	protected function setUp() {
		...
		$this->options['commit'] = $this->options['commit-test-ap-auto-approval-1'];
		...
		$this->options['autoapprove-label'] = 'Autoapproved Pull-Request';
	...
	private function cleanup_prs() {
		...
		foreach ( $prs_implicated as $pr_item ) {
			if ( (int) $pr_item->number !== (int) $this->options['pr-test-ap-auto-approval-1'] ) {
				printf(
