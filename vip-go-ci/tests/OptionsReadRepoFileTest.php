...
final class OptionsReadRepoFile extends TestCase {
	...
	var $options_patch_changed_lines
		= [
			'repo-owner' => null,
			'repo-name'  => null,
		];
	...
	public function testOptionsReadRepoFileIntTest5() {
		...
		vipgoci_options_read_repo_file(
			...
			[
				'phpcs-severity' => [
					'type'         => 'integer',
					'valid_values' => [ 500 ], // the value in options-file out of range
				],
			]
		);
	...
	public function testOptionsReadRepoFileOptionAllowedTest1() {
		...
		$this->options['post-generic-pr-support-comments'] = false;
		$this->options['phpcs-severity']                   = - 100;
		$this->options['phpcs-sniffs-exclude']             = [ 'MySniff.MySniffName' ];
		...
		$this->assertEquals(
			[ 'MySniff.MySniffName' ], // Should not have changed, is not configured
			$this->options['phpcs-sniffs-exclude']
		);
	}
